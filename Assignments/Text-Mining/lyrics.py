...
#hip_pop.append(PyLyrics.getLyrics('Run-D.M.C.','Sucker M.C.s')) #example of a song that did not show up
...
country.append(PyLyrics.getLyrics('Gary Allan', 'Songs about rain'))
...
f = open("hip_pop.txt", "w")
g_hp = [hip_pop]
f.write("\n".join(map(lambda x: str(x), g_hp)))
f.close()

#Unable to Open to other files
'''
f2 = open("country.py", "w")
g_c = [country]
f.write("\n".join(map(lambda y: str(y), g_c)))
f.close()

f3 = open("motown.py", "w")
g_m = [motown]
f.write("\n".join(map(lambda x: str(x), g_m)))
f.close()

f4 = open("boyband.py", "w")
g_bb = [boyband]
f.write("\n".join(map(lambda x: str(x), g_bb)))
f.close()

f5 = open("randb.py","w")
g_randb = [randb]
f.write("\n".join(map(lambda x: str(x), g_randb)))
f.close()
